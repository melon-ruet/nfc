...
public class ViewDragHelper {
    ...
    final class C00841 implements Interpolator {
        C00841() {
        }
    ...
    static {
        sInterpolator = new C00841();
    }

    private ViewDragHelper(Context context, ViewGroup viewGroup, Callback callback) {
        this.mActivePointerId = INVALID_POINTER;
        this.mSetIdleRunnable = new C00852();
        if (viewGroup == null) {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        i4 = Math.abs(i2);
        return Math.min(i4 > 0 ? Math.round(Math.abs(distanceInfluenceForSnapDuration / ((float) i4)) * 1000.0f) * EDGE_TOP : (int) (((((float) Math.abs(i)) / ((float) i3)) + 1.0f) * 256.0f), MAX_SETTLE_DURATION);
    }
